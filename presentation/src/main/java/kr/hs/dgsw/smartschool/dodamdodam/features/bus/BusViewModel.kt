...
class BusViewModel @Inject constructor(
    ...
    private val _addBusApplyState = MutableStateFlow(AddBusApplyState(isLoading = true))
    private val _updateBusApplyState = MutableStateFlow(UpdateBusApplyState(isLoading = true))
    ...
    init {
        combineLoadingVariable(isGetBusLoading, isGetMyBusLoading,isAddBusApplyLoading)
        getBusList()
    }
    ...
    private fun getBusList(){
        busUseCases.getBus(Unit).divideResult(
            isGetBusLoading,
            { _getBusListState.value = GetBusListState(busByDateList = it ?: emptyList<BusByDate>()) },
